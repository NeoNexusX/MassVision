...
若所需分析数据仍未上传，可点击右上角`Upload New Dataset`按钮上传所需分析数据，上传数据详细操作见[上传数据](./6.Dataset-Upload)。
...
- **可调参数**：
  - `Window`：窗口长度（正整数），默认 5。
    - 输入必须为正整数，偶数会自动调整为奇数以保证居中对齐。
    - 建议范围：5–15，可根据峰宽和噪声强度调节。窗口越大越平滑，但也可能模糊窄峰。
...
- **可调参数**：
  - `Scale`：归一化后的幅度缩放因子，输入值必须为有限非负数。若提供，强度会被乘以此值。
  - `Ref`：目标内标峰 *m/z* 值,若没有填写程序会自动取当前传入 m/z 数据的中间位置作为临时参考值。
  - `Ref Tolerance`：参考峰匹配容差，默认 0.1Da。例如 `Ref=500`、`Ref Tolerance=0.1` 可理解为在 `500 ± 0.1 Da` 的邻域内寻找参考信号。输入值必须为有限的非负数。
...
- **SNR**：信噪比阈值，默认 2.0。 `snr × noise` 为最小峰值，峰值低于此阈值的候选峰被丢弃。建议范围：1.5–5.0。较低的 snr 保留更多弱峰（含噪），较高的 snr 更保守。
...
- **Width**：局部极大值检测窗口半宽，默认 5。决定局部极大值检测的邻域范围。较大的 width 合并邻近小峰，较小的 width 保留更多峰。建议范围：3–11。根据仪器分辨率和期望峰密度调节。

![image-20260909164354430](https://official-oss.oss-cn-hongkong.aliyuncs.com/docs/20260909164354471.jpg_view)

#### 4.5峰对齐

- **Bin Function**：分辨率聚合策略,`"median"` |`"min"`|`"max"`|，默认是`"min"`，建议改为`"median"`，更加稳健。
  - `"median"`：中位数，稳健。
  - `"min"`：最细分辨率，参考轴更大。
  - `"max"`：最粗分辨率，参考轴更小。
...
创建成功后，自动跳转至`Workspace`,可在Workspace中查看处理进度，详细介绍见[Workspace](./8.Workspace)。
