<template>
  <div class="flex flex-col h-full">
    <div class="flex items-center gap-2 mb-2">
      ...
      <span class="text-xs text-base-content/40 ml-auto">{{ roi.type === 'freehand' ? 'Lasso' : 'Rect' }}</span>
    </div>
    <div ref="containerRef" class="relative flex-1 min-h-0 bg-base-200 rounded border border-base-300 overflow-hidden">
      <canvas ref="canvasRef" class="absolute inset-0 w-full h-full" style="image-rendering: pixelated;" />
    </div>
  </div>
</template>
...
import type { ConfirmedROI } from '@/composables/useROI'
...
watch(() => [props.roi, props.matrix, props.colormap, props.displayMin, props.displayMax],
