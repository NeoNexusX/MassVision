  ...
  <div class="flex flex-col gap-3 text-lg">
    ...
    <div v-if="rois.length">
      <div class="text-xl font-semibold text-base-content/50 mb-2 tracking-wide">
        ROIs
        <button class="text-base text-error ml-2 hover:underline" @click="$emit('clearAll')">Clear all</button>
      </div>
      <div class="space-y-2 max-h-64 overflow-y-auto">
        ...
        >
          <div class="flex items-center justify-between mb-1">
            <span class="font-semibold text-base" :style="{ color: roi.color }">{{ roi.label }}</span>
            <button class="text-sm text-base-content/30 hover:text-error" @click="$emit('delete', roi.id)">✕</button>
          </div>
          <span class="text-sm text-base-content/40">{{ roi.type === 'freehand' ? 'Lasso' : 'Rectangle' }}</span>
          <div v-if="roi.stats" class="mt-1 space-y-0.5 text-sm font-mono text-base-content/60">
            <div class="flex justify-between"><span>Pixels</span><span>{{ roi.stats.pixelCount }}</span></div>
            <div class="flex justify-between"><span>Mean</span><span>{{ fmt(roi.stats.mean) }}</span></div>
            <div class="flex justify-between"><span>Std</span><span>{{ fmt(roi.stats.std) }}</span></div>
            <div class="flex justify-between"><span>Min</span><span>{{ fmt(roi.stats.min) }}</span></div>
            <div class="flex justify-between"><span>Max</span><span>{{ fmt(roi.stats.max) }}</span></div>
          </div>
        </div>
      </div>
    </div>
