<template>
  ...
  >
    ...
    <template v-if="tool === 'rectangle' && draftReady">
      <div class="absolute w-2.5 h-2.5 bg-white border-2 border-red-500 rounded-full -translate-x-1/2 -translate-y-1/2 cursor-nwse-resize z-30"
        :style="handlePos.tl" @mousedown.prevent.stop="startHandleDrag('tl', $event)" />
      <div class="absolute w-2.5 h-2.5 bg-white border-2 border-red-500 rounded-full -translate-x-1/2 -translate-y-1/2 cursor-nesw-resize z-30"
        :style="handlePos.tr" @mousedown.prevent.stop="startHandleDrag('tr', $event)" />
      <div class="absolute w-2.5 h-2.5 bg-white border-2 border-red-500 rounded-full -translate-x-1/2 -translate-y-1/2 cursor-nesw-resize z-30"
        :style="handlePos.bl" @mousedown.prevent.stop="startHandleDrag('bl', $event)" />
      <div class="absolute w-2.5 h-2.5 bg-white border-2 border-red-500 rounded-full -translate-x-1/2 -translate-y-1/2 cursor-nwse-resize z-30"
        :style="handlePos.br" @mousedown.prevent.stop="startHandleDrag('br', $event)" />
    </template>
  </div>
</template>
...
import type { ROIType, DraftROI, ROIPoint } from '@/composables/useROI'
...
const handlePos = computed(() => {
  ...
  return {
    tl: { left: (t.ox + xmin * t.scale) / t.W * 100 + '%', top: (t.oy + ymin * t.scale) / t.H * 100 + '%' },
    tr: { left: (t.ox + xmax * t.scale) / t.W * 100 + '%', top: (t.oy + ymin * t.scale) / t.H * 100 + '%' },
    bl: { left: (t.ox + xmin * t.scale) / t.W * 100 + '%', top: (t.oy + ymax * t.scale) / t.H * 100 + '%' },
    br: { left: (t.ox + xmax * t.scale) / t.W * 100 + '%', top: (t.oy + ymax * t.scale) / t.H * 100 + '%' },
  }
})
...
function getScale() {
  ...
  if (!el || !props.imageWidth || !props.imageHeight)
...
function emitRect() {
  emit('draft-updated', {
    type: 'rectangle',
    rect: { x0: Math.min(rect.x0, rect.x1), y0: Math.min(rect.y0, rect.y1), x1: Math.max(rect.x0, rect.x1), y1: Math.max(rect.y0, rect.y1) },
    path: [],
  })
}
...
function onDown(e: MouseEvent) {
  ...
  if (props.tool === 'rectangle' && draftReady.value) {
    ...
    if (p.x >= xmin && p.x <= xmax && p.y >= ymin && p.y <= ymax) {
      isMoving = true
      drawing = true
      const dx0 = p.x - rect.x0, dy0 = p.y - rect.y0
      const dx1 = p.x - rect.x1, dy1 = p.y - rect.y1
  ...
  drawing = true
  if (props.tool === 'rectangle') {
    rect.x0 = p.x; rect.y0 = p.y; rect.x1 = p.x; rect.y1 = p.y
...
function startHandleDrag(corner: 'tl' | 'tr' | 'bl' | 'br', e: MouseEvent) {
  ...
  const onUp = () => { dragCorner = null; document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp) }
  document.addEventListener('mousemove', onMove)
  document.addEventListener('mouseup', onUp)
}
...
function clearAll() {
  ...
  drawing = false
  isMoving = false
  rect.x0 = 0; rect.y0 = 0; rect.x1 = 0; rect.y1 = 0
  freePath.value = []
  document.removeEventListener('mousemove', onDrawMove)
  document.removeEventListener('mouseup', onDrawUp)
  initCanvas()
...
onMounted(() => {
  ...
  if (el) {
    initCanvas()
    ro = new ResizeObserver(() => { initCanvas(); redraw() })
    ro.observe(el)
  }
})
onBeforeUnmount(() => {
