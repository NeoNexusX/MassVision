        ...
        <div>
          ...
          <div class="mt-2">
            <div v-for="m of selectedMethods" :key="m" class="mb-2">
              ...
              <input class="input input-bordered w-full mt-1" v-model="params[m]" placeholder="parameter JSON or value" />
            </div>
          </div>
        </div>
      </div>
...
const datasets = ref([{ id: 'DS1', name: 'Dataset 1' }, { id: 'DS2', name: 'Dataset 2' }])
const methods = [
  'TIC Normalization',
  'RMS Normalization',
...
watch(selectedMethods, (n) => {
  n.forEach(m => { if (!(m in params)) params[m] = '' })
})

const start = () => {
  // basic payload
  const payload = { id: 't' + Date.now(), name: `Process ${selectedDataset.value}`, dataset: selectedDataset.value, methods: [...selectedMethods.value], status: 'Queued', progress: 0, created: new Date().toISOString(), params: { ...params } }
