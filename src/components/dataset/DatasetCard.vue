...
<template>
  <div class="group flex items-center p-4 bg-base-100 dark:bg-slate-800 rounded-xl shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 border border-base-300">
    <!-- Left: Image -->
    <div class="flex-none w-[120px] h-[120px] rounded-lg overflow-hidden bg-base-200 flex items-center justify-center border border-base-300">
      <img 
    ...
    <div class="flex-1 flex flex-col gap-1 px-4 min-w-0">
      <div class="flex items-center gap-2">
        <h3
          class="font-bold text-lg text-base-content truncate cursor-pointer hover:text-primary dark:hover:text-indigo-400 transition-colors"
    ...
    <div class="flex-none w-[140px] flex flex-col gap-2 border-l border-base-300 pl-4 items-start justify-center">
      ...
      <template v-if="isMyDataset">
        <div class="w-full h-px bg-base-200 my-1"></div>
        <button class="flex items-center gap-2 text-sm text-base-content hover:text-primary dark:hover:text-indigo-400 transition-colors group/btn" @click="$emit('edit', dataset.id)">
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
              <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
           </svg>
           <span>Edit</span>
        </button>
        <button class="flex items-center gap-2 text-sm text-error hover:text-error transition-colors group/btn" @click="$emit('delete', dataset.id)">
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
