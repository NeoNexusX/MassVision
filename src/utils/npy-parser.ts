...
type TypedArray = Float32Array | Float64Array | Int32Array | BigInt64Array | Uint8Array | Uint16Array | Uint32Array

const DTYPE_MAP: Record<string, new (len: number) => TypedArray> = {
  '<f4': Float32Array,
  '<f8': Float64Array,
  '<i4': Int32Array,
  '<i8': BigInt64Array,
...
function parseNpy(buffer: ArrayBuffer): NpyResult {
  ...
  if (Ctor === BigInt64Array) {
    data = new BigInt64Array(rawBuffer)
  } else {
    data = new Ctor(rawBuffer)
  }
