<template>
  <div class="flex flex-wrap items-center gap-3 px-1">
    <div class="flex items-center gap-2">
      ...
      <span class="badge badge-sm" :class="statusClass">{{ status }}</span>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 text-xs">
      <span
        v-for="item in chips"
        :key="item.label"
        class="inline-flex items-center gap-1.5 bg-base-200 rounded-full px-2.5 py-1"
      >
    ...
    <div v-if="methods?.length" class="flex flex-wrap items-center gap-1.5 ml-auto">
      <span v-for="m in methods" :key="m" class="badge badge-outline badge-sm">{{ m }}</span>
    </div>
  </div>
</template>
