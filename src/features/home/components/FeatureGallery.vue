    ...
    <div class="pt-4">
      <Transition name="card-fade" mode="out-in">
        <div :key="active">
          ...
          >{{ activeItem.word }}</h3>
          <p v-if="activeItem.title" class="mt-4 kawaru-text-112 font-semibold text-base-content">{{ activeItem.title }}</p>
          <p v-if="activeItem.desc" class="mt-4 text-justify kawaru-text-95 leading-[1.5] text-base-content/60">{{ activeItem.desc }}</p>
        </div>
      </Transition>
    </div>
  </div>
