...
export async function listCollections(page: number, size: number): Promise<CollectionListResponse> {
  const body = await unwrap<any>(() =>
    auth_api.post('/collections/list', {}, { params: { page, size } }),
  )
...
export async function listAllCollections(
  ...
  const body = await unwrap<any>(() =>
    auth_api.post('/collections/list_all', {}, { params: { page, size } }),
  )
  return { meta: toListMeta(body?.meta), data: normalizeList(body).map(mapCollectionSummary) }
}
...
export async function removeMembers(id: number, filePublicIds: string[]): Promise<MemberRemovalResult> {
  const body = await unwrap<any>(() =>
...
export async function reorderMembers(id: number, filePublicIds: string[]): Promise<CollectionDetail> {
  const body = await unwrap(() =>
