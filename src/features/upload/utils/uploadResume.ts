...
const STORAGE_KEY = 'oss_upload_session_v2'
/** 迁移前版本的会话 key：字段是数字 fileId，无法推导出 publicId —— 只清理，不迁移 */
const LEGACY_STORAGE_KEY = 'oss_upload_session'
...
export interface UploadSession {
  datasetName: string
  fileHash: string
  /** 预检返回的 public_id（16 位字符串）。旧会话存的是数字 fileId，已整体失效 */
  filePublicId: string
  /**
...
/** 清掉旧版本会话：旧会话存的是数字 fileId，publicId 迁移后无法安全续传，只能失效 */
...
export function loadUploadSession(): UploadSession | null {
  ...
  try {
    ...
    // 但会话本身仍然有效（保留 filePublicId 与 STS，下次续传重开一轮 multipart）。
    if (!data.filePublicId || !data.ossPath || !data.fileHash) return null
...
/** 重置分片进度，让续传重新开一轮 OSS multipart（保留 STS 凭证与 filePublicId） */
