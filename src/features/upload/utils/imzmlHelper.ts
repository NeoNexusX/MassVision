...
import { auth_api, formatErrorMessage } from '@/shared/api/httpClient'
...
export function validateImzmlFilePair(files: File[] | FileList): ImzmlFilePair {
  ...
  if (fileArray.length !== 2) {
    throw new Error(
      `File count error: Please select exactly 2 files, currently selected ${fileArray.length} .`,
    )
  }
 ...
 */
export function createFileChunks(zipFile: File, chunkSize?: number): ChunkPlan[] {
...
export async function preflightFile(
  ...
  try {
    ...
    if (error.response && error.response.status === 400) {
      ...
      throw new Error(
        `Preflight failed (400 Invalid param): ${formatErrorMessage(error.response.data)}`,
      )
    }
    throw error
  }
 ...
 */
export async function validateAndMergeChunks(
  fileId: string,
  signal?: AbortSignal,
): Promise<ChunkStatusResponse> {
  const response = await auth_api.post(`/files/chunked/${fileId}/validate_and_merge`, null, {
    signal,
  })
  return response.data || response
}
  ...
  while (attempt <= maxRetries) {
    ...
    try {
      ...
      if (attempt > maxRetries) {
        throw new Error(
          `Failed to upload chunk ${chunk.partNumber} after ${maxRetries} retries: ${errMsg}`,
        )
      }
...
export async function uploadChunks(
  ...
  const chunkedAlreadyUploadedBytes =
    totalBytes - pendingChunks.reduce((sum, c) => sum + c.chunkSize, 0)
  ...
  const reportProgress = () => {
    ...
    inProgressMap.forEach((bytes) => {
      currentInflightTotal += bytes
    })
  ...
  const worker = async () => {
    while (chunkQueue.length > 0) {
      ...
      try {
        ...
        if (isLastPart) {
          console.warn(
            `Last chunk ${chunk.partNumber} upload failed and will be skipped:`,
            err?.message || err,
          )
  ...
  const workers = Array.from({ length: Math.min(concurrency, pendingChunks.length) }, () =>
    worker(),
  )
