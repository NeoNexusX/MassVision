<template>
  <div class="flex-1 w-full flex items-center justify-center p-8 bg-base-200/30">
    <div class="card max-w-5xl w-full bg-base-100 shadow-2xl rounded-2xl overflow-visible border border-base-200">
      <div class="flex flex-col lg:flex-row relative items-stretch">

        <!-- Left Side: Account Credentials -->
        <div class="w-full lg:w-1/2 p-8 md:p-10 flex flex-col flex-1 min-h-0 border-b lg:border-b-0 lg:border-r border-base-200">
          <!-- Row 1 -->
          <div class="min-h-[72px] mb-4">
            <h2 class="text-3xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent w-fit">
              Create Account
            </h2>
            <p class="text-base-content/60 mt-2 text-sm">
              Join MassFlow for scientific data analysis
            </p>
          </div>
        ...
        <div class="w-full lg:w-1/2 p-8 md:p-10 pb-8 flex flex-col flex-1 min-h-0 bg-base-50/50 dark:bg-base-200/20">
          <!-- Row 1 -->
          <div class="min-h-[72px] mb-4">
...
const patterns = {
  // Put '-' escaped inside character classes to avoid invalid-class errors
...
const { count: countdown, isActive: isCountdownActive, isExhausted, start: startCountdown } =
  ...
  switch (field) {
    case 'username':
      errors.username = !val
        ...
        : !new RegExp(patterns.username).test(val)
    ...
    case 'email':
      errors.email = !val
        ? 'Email is required'
        : !new RegExp(patterns.email).test(val)
    ...
    case 'password':
      errors.password = !val
        ...
        : !new RegExp(patterns.password).test(val)
          ? 'Min 8 chars, letters & numbers required'
          : ''
      validatePasswordStrength()
      break
    case 'confirm_password':
      errors.confirm_password = !val
      ...
      break
    case 'verify_code':
      errors.verify_code = !val
        ? 'Code is required'
        : !new RegExp(patterns.verify_code).test(val)
          ? 'Must be 6 digits'
          : ''
      break
    case 'institution':
...
const register = async () => {
  ...
  if (!form.username || !form.email || !form.password || !form.institution || !form.position || !form.region) {
