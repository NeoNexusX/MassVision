...
test.describe('New Analysis', () => {
  ...
  test('switching dataset updates summary panel', async ({ page }) => {
    ...
    const firstName = await firstLi.locator('.font-medium').textContent()
    await firstLi.click()
    ...
    const secondName = await secondLi.locator('.font-medium').textContent()
  ...
  test('selecting a dataset updates summary panel', async ({ page }) => {
    ...
    const datasetName = await nameEl.textContent()
